...
function UpcommingEvents(props) {
  ...
  var e_dt = props.event.endTime ? props.event.endTime : null;
  ...
  const st_date = props.event.startTime ? str_dt(props.event.startTime) : null;
  const ed_date = props.event.endTime ? str_dt(props.event.endTime) : null;
  if (st_date === ed_date) {
    e_dt = null;
  }
  var startDate = props.event.date;
  ...
  var e_time_s = tConvert(getTime(props.event.startTime));
  var e_time_e = tConvert(getTime(props.event.endTime));
        ...
        <div className="d-flex mb-3">
          ...
          <div className="flex-shrink-0">
            <small className="badge badge-soft-primary ms-auto">
              {new Date(props.event.startTime).toLocaleTimeString()}  {new Date(props.event.endTime).toLocaleTimeString()}
            </small>
          </div>
        </div>
